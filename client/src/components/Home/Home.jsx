  ...
  if (loading) {
    ...
    return (
      ...
      <ul className={style.orderList}>
        ...
        ?currentGames && currentGames?.map(games => (
            <GameCard key = {games.id} games = {games} />
        ))
        : <h1 className={style.orderLoading}><AiOutlineLoading3Quarters setLoading={setLoading} className={style.loading}/></h1>
        }
      </ul>
      <div className={style.pagination}>
    <Pagination gamesPerpage = {gamesPerpage} allGames = {allGames.length}  paginado={paginado}/>
    </div>
      </div>
      </div>
    )
